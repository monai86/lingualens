...
def test_downgrade_refuses_to_delete_existing_evidence_jobs(monkeypatch) -> None:
    ...
    with TemporaryDirectory(prefix="lingualens-assessment-v2-downgrade-") as temp_dir:
        database_path = Path(temp_dir) / "assessment-v2.db"
        monkeypatch.setenv("LINGUALENS_ASSESSMENT_DATABASE_URL", f"sqlite:///{database_path}")
        get_settings.cache_clear()
        upgrade_assessment_database()
        try:
            with sqlite3.connect(database_path) as connection:
                ...
                connection.execute(
                    ...
                    "(processing_run_id, organization_id, recording_id, assessment_id, "
                    "transcript_revision_id, stage, state, idempotency_key, attempt_count, "
                    "available_at, error_code, created_at, updated_at) "
                    "VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, NULL, "
                    "CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                    (
                        "processing_run_downgrade_guard",
                        "org_opaque_01",
                        "assessment_opaque_01",
                        "transcript_revision_opaque_01",
                        "evidence_extraction",
                        "queued",
                        "evidence:v1:downgrade-guard",
                        0,
                    ),
                )

            with pytest.raises(RuntimeError, match="evidence_processing_downgrade_blocked"):
                downgrade_assessment_database("0006_evidence_profiles")
        finally:
            get_settings.cache_clear()
